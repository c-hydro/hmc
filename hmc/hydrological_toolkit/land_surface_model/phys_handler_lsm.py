...
class LSMHandler:
    ...
    def execute(self, dset_data: xr.Dataset, dset_state: xr.Dataset) -> None:

        # get variables geo
        var_s = get_variable_data(self.dset_geo, var_name='s', var_mandatory=True)

        # get variables physics
        var_vtot = get_variable_data(dset_state, var_name='vtot', var_mandatory=True)
